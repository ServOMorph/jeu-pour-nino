...
func _build_ui() -> void:
	...
	_skill_label.position = Vector2(PX + 820, PY + 28)
	_skill_label.size = Vector2(340, 48)
	...
	var hint := Label.new()
	hint.text = "A: maitriser   B: fermer"
	hint.position = Vector2(PX + 24, PY + 708)
	...
	for i in range(27):
		...
		bg.position = Vector2(PX + 20, PY + 96 + i * ROW_H)
		bg.size = Vector2(PW - 40, ROW_H - 8)
		add_child(bg)
		_row_bgs.append(bg)

		var lbl := Label.new()
		lbl.position = Vector2(PX + 36, PY + 106 + i * ROW_H)
...
func _refresh() -> void:
	...
	for i in _row_labels.size():
		var visible_row := i < _visible_recipes.size()
		_row_bgs[i].visible = visible_row
		_row_labels[i].visible = visible_row
		if not visible_row:
			continue
		var recipe := _visible_recipes[i]
		var id := String(recipe.get("id", ""))
		...
		_row_labels[i].text = "%s - %s - %s" % [String(recipe.get("name", id)), status, discovery]
		_row_bgs[i].color = Color(0.24, 0.22, 0.18) if i == _selected else Color(0.16, 0.15, 0.12)
		if mastered:
			_row_labels[i].modulate = Color(0.45, 0.82, 0.45)
		elif MetaState.skill_points >= skill_cost:
			_row_labels[i].modulate = Color(0.95, 0.92, 0.8)
		else:
