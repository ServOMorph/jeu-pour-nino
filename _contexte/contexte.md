...
La sheet runtime de course du joueur est conservée. Les essais ImageGen et WanGP de refonte complète sont rejetés : alpha/grille insuffisants pour ImageGen, profil droit instable pour WanGP.
La prochaine session doit demander à Astra un workflow de génération complet, contrôlable et validable pour les 16 frames de `player/run`.
Le boot headless de `hub.tscn` est vert après la refonte visuelle du 2026-09-13.
Prochaine étape : concevoir ce workflow, puis vérifier le HUB en jeu et valider J1 avant la Phase 4.
