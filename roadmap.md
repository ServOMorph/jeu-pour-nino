...
**Interface, narration, audio, art** — Écran d'équipement manuel (pause), barres de vie au-dessus des ennemis (pas de chiffres de dégâts flottants, pas de minimap), Grimoire accessible au HUB uniquement. Narration par textes courts aux moments clés (pas de dialogues), personnage anonyme (pas nommé Nino), textes scénarisés en fondu simple. **Audio définitif reporté à la toute fin du développement** (Phase 10) — garder les placeholders sonores actifs via `AudioManager` pendant tout le développement, ne reporter que le remplacement par des assets soignés. Liste fermée de bruitages indispensables, aucune voix. Tuiles/décors en 2D standard (résolution native 1920×1080, pas de grille pixel imposée — pivot acté, voir `plan_graphismes_standard_2d.md`), 2-3 couches de parallax, 5 paliers de shaders de cicatrices, priorité de remplacement des placeholders : joueur > ennemis > boss > tilesets > UI.
...
- [x] **Armes à distance (Q016)** : `weapons.json` porte `type: ranged|melee`, `projectile_speed`, `cooldown` sur `arc_bois`/`arbalete_fer`/`arc_celeste`. Scène `scenes/player/player_projectile.tscn` + `player_projectile.gd` (placeholder Polygon2D, pas de sprite). Pas de bouton manette dédié : le bouton `attack` existant (gâchette droite) déclenche le tir si l'arme équipée est `ranged`, sinon le mêlée — un seul type actif à la fois (slot unique Q017). Usage illimité, pas de munitions. Tir géré par `player.gd._fire_ranged()`. Validation manuelle en jeu restant à faire par Nino.
- [x] **Barème PC (Q044)**, `game/data/progression.json` branché : biome visité 1 PC, salle secrète 1 PC, élite vaincu 1 PC, boss de biome vaincu 2 PC, résurrection réussie 1 PC, victoire finale (Miroir) +3 PC bonus, run raté ×0.5 sur le total. Pas de PC par salle normale explorée.
...
- [x] Gain de PC en fin de run — compteurs génériques `RunState.run_counters` (`increment_counter`/`get_counter`/`set_counter_flag`/`get_counters`, reset avec `RunState.reset()`), incrémentés par `level.gd` (`biomes_visited` en `_ready`, `bosses_defeated` en `_on_boss_died`, flag `run_failed` en `_on_player_died`). Calcul dans la fonction pure `Progression.compute_skill_points(counters, bareme)` (`scripts/progression.gd`), appelée via `level.gd._award_skill_points()` aux deux points de fin de run existants, avant `SaveManager.save_meta()`. Tant que salles/biomes multiples n'existent pas (Phases 3-4), les compteurs valent 0 ou 1 — le barème fonctionne déjà correctement.
...
- [x] Validation manuelle du flux Phase 2 déjà branché : ouvrir le Grimoire en mode dev, maîtriser une recette, lancer une partie, crafter, équiper via pause, vérifier HUD/consommable. **Complète** (2026-07-12, voir `tests_manuels.md` racine) : toutes les sections testées OK, aucune anomalie bloquante. `discover_recipe()` branché sur victoire boss (`level.gd._on_boss_died()` → `RecipeCatalog.discover_by_trigger()`) — seules les 3 recettes `Victoire Gardien du Voile` sont découvrables à ce stade ; les 17 recettes `Salle B1-B4`/`Porteur` attendent les phases 4/7a-c/8 (biomes multiples, porteurs).
...
- [x] Nouveau `tests/test_craft.gd` (12 tests) : filtrage maîtrisées, filtrage par tier d'établi, craft débite les matériaux (mono et multi-matériaux), échec sans matériaux, starters mastered au bootstrap, découverte → maîtrise (coût PC) succès/échec, `Progression.compute_skill_points` (cas basique, bonus victoire, multiplicateur run raté, valeurs réelles du barème). Complété par 3 tests de compteurs dans `tests/test_run_state.gd`.
...
### Tâches
- [x] Créer `scenes/levels/hub.tscn` + `scripts/hub.gd` : point central, 4 portails (`scripts/hub_portal.gd`, modèle `workbench.gd` — les 3 biomes non implémentés sont `locked`, prompt « EN CONSTRUCTION »), player sans ennemis, config `game/data/hub.json`. Décor plein écran `hub_decor_portals_v2.png` branché en fond ; portes intégrées sans rectangles ni noms placeholder, établi à droite.
- [x] `title.gd._start_game()` : passe par `GameFlow.start_run()` (**seul point de `RunState.reset()`**) puis `GameFlow.return_to_hub()`. Le menu dev conserve les raccourcis « BIOME DIRECT »/« ATELIER »/« TEST BOSS » via `GameFlow.enter_biome()`.
- [x] Chargement de niveau paramétré : autoload `GameFlow` (`scripts/game_flow.gd`) porte `next_biome_id` ; `level.gd._ready()` charge `res://data/biomes/<id>.json`. `level.json` → `data/biomes/biome1.json` ; `biome1.tscn` → `biome.tscn` (scène générique).
- [x] Retour au HUB : victoire boss → écran bref (`scripts/biome_cleared.gd`) puis `hub.tscn`. La mort reste une fin de run (`end_screen`), « REESSAYER » relance un nouveau run depuis le HUB. Plus aucun `get_tree().quit()` ni `reload_current_scene()`.
- [x] **Sortie volontaire (Q006)** : portail « RETOUR HUB » (`exit_portal` dans le JSON de biome), récolte conservée.
- [x] **Soin au HUB (Q024)** : `player.heal_full()` à l'entrée du HUB.
- [x] Boss de biome vaincu : `RunState.defeated_bosses` (`mark_boss_defeated`/`is_boss_defeated`) — le boss n'est plus spawné dans le run en cours, le biome reste explorable.
- [x] Accès depuis le HUB : stèle Grimoire (déblocage PC) et établi tier 1.
- [x] Tests (`tests/test_game_flow.gd`, 9 tests) : chargement du bon fichier de biome, reset unique par run, revisite d'un biome comptée une seule fois, conservation matériaux/équipement sur aller-retour, PC de fin de run (run raté vs réussi).
- [ ] **Validation en jeu** (`python run_game.py`) : HUB → biome → retour HUB → re-entrée, avec conservation de la récolte et de l'équipement.
...
- [ ] `level.gd` : consommer le Dictionary généré (seed tirée au lancement du run, conservée dans `RunState` — nécessaire à la Phase 5). Écran fixe par salle (1920×1080, pas de scroll interne, Q038) — confirmé, pas de caméra dynamique à gérer.
...
- [ ] Recenser tileset/décors biome 1 dans `game_art/backlog_art.md` — 2D standard résolution native (pivot pixel art abandonné, Q068 révisée), 2-3 couches de parallax (Q069). **[game_art]**
...
- [ ] Recenser par biome dans `game_art/backlog_art.md` : sprites ennemis, boss, décors, tileset (2D standard résolution native, pivot pixel art abandonné, Q068 révisée). **[game_art]**
