...
### Tâches
- [x] Créer `scenes/levels/hub.tscn` + `scripts/hub.gd` : point central, 4 portails (`scripts/hub_portal.gd`, modèle `workbench.gd` — les 3 biomes non implémentés sont `locked`, prompt « EN CONSTRUCTION »), player sans ennemis, config `game/data/hub.json`. Décor `hub_decor.png` (livré par game_art) branché en fond.
